...
def generate_solutions(board, me):
    ...
    for col1, col2 in itertools.product(range(7), range(7)):
        ...
        for upper1, upper2 in itertools.product([row for row in range(6) if row % 2 == 0 and row != 0
                                                 and board[col1][row].state.value == computer.State.empty.value 
                                                 and board[col1][row - 1].state.value == computer.State.empty.value],
                                                [row for row in range(6) if row % 2 == 0 and row != 0
                                                 and board[col2][row].state.value == computer.State.empty.value 
                                                 and board[col2][row - 1].state.value == computer.State.empty.value]):
